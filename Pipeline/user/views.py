...
def allproject_view(request):
    projects = Shot2.objects.values('project_name').distinct().order_by('project_name')
...
def issueshot_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            obj.save()
            messages.success(request, 'Shot issued successfully!')
...
def deleteissuedshot_view(request, pk):
    ...
    shot_obj.delete()
    messages.success(request, 'Issued shot removed successfully!')
    return redirect('viewissuedshot')
